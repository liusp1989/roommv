...
public class HtmlInfoController {
    public static final Logger logger = Logger.getLogger(HtmlInfoController.class);
    @Resource(name = "htmlInfoService")
    private IHtmlInfoService htmlInfoService;

    @RequestMapping("list")
    public String list(ModelMap model) {
	List<Object> htmlInfos = htmlInfoService.getAllObjectList(HtmlInfo.class, null);
    ...
    public String edit(@PathVariable String id, ModelMap model) throws UnsupportedEncodingException {
	DetachedCriteria criteria = DetachedCriteria.forClass(HtmlInfo.class);
	Criterion criterion = Restrictions.eq("id", id);
	criteria.add(criterion);
	HtmlInfo htmlInfo = (HtmlInfo) htmlInfoService.findObjects(criteria).get(0);
	// if (!StringUtils.isEmpty(htmlInfo.getContent())) {
	// htmlInfo.setContent(URLEncoder.encode(htmlInfo.getContent(),
	// "UTF-8"));
	// }
	model.put("htmlInfo", htmlInfo);
	return "common/editor";
    ...
    public String audit(@ModelAttribute("htmlInfo") HtmlInfo htmlInfo, ModelMap modelMap)
	    throws UnsupportedEncodingException {
	DetachedCriteria criteria = DetachedCriteria.forClass(HtmlInfo.class);
	Criterion criterion = Restrictions.eq("id", htmlInfo.getId());
	criteria.add(criterion);
	HtmlInfo orightmlInfo = (HtmlInfo) htmlInfoService.findObjects(criteria).get(0);
	...
	try {
	    ...
	    if (RoommvConstant.AuditStatus.AUDIT_PASS.getCode().equals(htmlInfo.getAuditStatus())) {
		String htmlId = htmlInfo.getHtmlId();
		String htmlName = htmlId + ".html";
		File tempFile = new File(RoommvConstant.HTML_FILES_TEMPPATH + htmlName);
		String destFilePath = RoommvConstant.HTML_FILES_PATH + htmlName;
		File destFile = new File(destFilePath);
		FileUtils.copyFile(tempFile, destFile);
		Analyzer analyzer = new PaodingAnalyzer();
		HtmlIndexer htmlIndexer = new HtmlIndexer(RoommvConstant.HTML_INDEXES_PATH,
			destFilePath, OpenMode.APPEND);
    ...
    public String view(@PathVariable String id, ModelMap model) throws UnsupportedEncodingException {
	DetachedCriteria criteria = DetachedCriteria.forClass(HtmlInfo.class);
	Criterion criterion = Restrictions.eq("id", id);
	criteria.add(criterion);
	HtmlInfo htmlInfo = (HtmlInfo) htmlInfoService.findObjects(criteria).get(0);
	...
	try {
	    ...
	    Criterion criterion = Restrictions.eq("id", id);
	    criteria.add(criterion);
	    List<HtmlInfo> htmlInfos = htmlInfoService.findObjects(criteria);
    ...
    @RequestMapping("save")
    public String save(@ModelAttribute("htmlInfo") HtmlInfo htmlInfo, ModelMap model)
	    throws IOException {
	...
	try {
	    ...
	    titleBuilder.append("<!DOCTYPE HTML PUBLIC '-//W3C//DTD HTML 4.01 Transitional//EN'\n");
	    titleBuilder.append("'http://www.w3.org/TR/html4/loose.dtd'>\n");
	    titleBuilder.append("<head>\n");
	    titleBuilder
		    .append("<meta http-equiv='Content-Type' content='text/html; charset=utf-8'/>\n");
	    titleBuilder.append("<title>").append(URLDecoder.decode(title, "UTF-8"))
		    .append("</title>\n");
	    titleBuilder
	    ...
	    bodyBuilder.append(URLDecoder.decode(content, "UTF-8"));
